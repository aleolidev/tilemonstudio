...
from backend.color_utilities import pixmap_to_pil, pil_to_pixmap, quantize
import colorsys
...
class Tileset:
    def __init__(self, is_transparent: bool, raw_pixmap):
        self.is_transparent = is_transparent
        self.rap_pixmap = raw_pixmap
        self.bg_color = self.get_bg_color(np.array(pixmap_to_pil(raw_pixmap)))
        self.to_process_tileset = self.rgba_to_rgb_and_background_color(pixmap_to_pil(raw_pixmap), self.bg_color)
        self.tiles = {}
        self.slice_tileset()
    ...
    def slice_tileset(self):
        self.tiles = {}
        image = self.to_process_tileset
        void_tile = pil.Image.new("RGB", (8, 8), self.bg_color)
    ...
    def generate_palettes(self):
        self.slice_tileset()
        tile_pals = list(map(lambda x: x.pal, self.tiles.values()))
    ...
    def generate_global_palette(self):
        g_palette = {color: self.get_tiles_per_color(color) for color in map(lambda x: x[1], self.to_process_tileset.getcolors())}
        return g_palette
    ...
    def reduce_duplicate_tiles(self, image, refactor, remove_flips):
        if refactor:
            tiles = [pil.Image.new("RGB", (8, 8), self.bg_color)]
            for y in range(image.size[1] // 8):
                for x in range(image.size[0] // 8):
            ...
            if tiles.shape[0] % 16 > 0:
                height += 8
            reduced_tileset = pil.Image.new("RGB", (width, height), self.bg_color)
    ...
    def rgba_to_rgb_and_background_color(self, img, color):
        if len(img.getcolors()[0][1]) > 3:
            image = pil.Image.new("RGB", img.size, color)
